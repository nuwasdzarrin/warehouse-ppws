                ...
                <form class="form">
                    <div class="ibox">
                        <div class="ibox-title">
                            <h5>Buat baru Transaksi Masuk</h5>
                        </div>
                        <div class="ibox-content">
                            ...
                            <div class="form-group" :class="{'has-error': formErrors && formErrors.has('amount')}">
                                ...
                                <div style="padding: 2px 0;" v-if="amount">Stok: {{stock-originTransaction.amount}} + {{amount}} = {{ Number(stock-originTransaction.amount) + Number(amount) }}</div>
...
export default {
    ...
    methods: {
        ...
        settingValidation: function () {
            ...
            this.formValidator.attach({name: 'amount', rules: `min_value:1`, alias: 'jumlah'});
            ...
            this.formValidator.validateAll({
                ...
                if (value) {
                    ...
                    axios({
                        ...
                        url: `/api/transaction_ins/${this.originTransaction.id}`,
                        data: formData,
                    }).then((res) => {
                        this.$set(this,'isLoading',false);
                        toastr.options.progressBar = true;
                        toastr.success('Transaksi Masuk Berhasil Di Perbaharui', this.$t('Success'));
                        window.location.replace('/transaction_ins');
                    }).catch((err) => {
                        this.$set(this,'isLoading',false);
                        toastr.options.progressBar = true;
                        toastr.error(err.response.data.message ? err.response.data.message : err.response.data.exception.split('\\').pop(), this.$t('Error'));
