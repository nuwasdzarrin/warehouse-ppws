                    ...
                    <div class="ibox">
                        ...
                        <div class="ibox-content">
                            ...
                            <div class="form-group" :class="{'has-error': formErrors && formErrors.has('amount')}">
                                ...
                                <div style="padding: 2px 0;" v-if="amount">Stok: {{stock}} + {{amount}} = {{ Number(stock) + Number(amount) }}</div>
...
export default {
    ...
    methods: {
        ...
        settingValidation: function () {
            ...
            this.formValidator.attach({name: 'amount', rules: `required|min_value:1`, alias: 'jumlah'});
            ...
            this.formValidator.validateAll({
                ...
                if (value) {
                    ...
                    axios({
                        ...
                        url: '/api/transaction_ins',
                        data: formData,
                    }).then((res) => {
                        this.$set(this,'isLoading',false);
                        toastr.options.progressBar = true;
                        toastr.success('Transaksi Masuk Berhasil Dibuat', this.$t('Success'));
                        if (create) window.location.reload();
                        else window.location.replace('/transaction_ins');
                    }).catch((err) => {
                        this.$set(this,'isLoading',false);
                        toastr.options.progressBar = true;
                        toastr.error(err.response.data.message ? err.response.data.message : err.response.data.exception.split('\\').pop(), this.$t('Error'));
